...
const RiskAssessment = ({formData, handleFormData, setFormData}) => {
  ...
  useEffect(() => {
    axios.get("http://localhost:8000/api/risk-management/get-questions-dict/flooding/").then((resp) => {
      const questions = resp.data[0]["questions"]
      manageQuestion(questions, setQuestion, setCurrentQuestion)
    })
  }, [])

  const hidden = currentQuestion === "Low Risk" || currentQuestion === "Moderate Risk" || currentQuestion === "Minimal Risk" || currentQuestion === "High Risk"

  return (
    <>
      <h1>Risk Assessment Detail</h1>
      <p>{currentQuestion}</p>
      <button disabled={hidden} onClick={() => {
      ...
      <hr/>
      <h1>Add Contact</h1>
      <input type="text" placeholder={"phonenumbers , separated"} name={"phonenumbers"} onChange={handleFormData}/>
      <hr/>
    </>
  );
};
